...
define("MODULO","contato_categoria");
...
switch ($_GET['Op'])
{
	case "Fil": ##Filtro
		...
		$Form = new ContatoCategoriaForm();
		$ContCategoria = new ContatoCategoria();
		$FPHP = new FuncoesPHP();
		...
		try 
		{
			...
			echo($FPHP->formataErro($Form->getErro()).$ContCategoria->filtrar($Form));
		}
		catch (Exception $E)
		{
			echo($E->getMessage());
		...
		{		
			...
			else 
			{
				...
				{		
					...
					$ContCategoria->getDados($Id,"POST");
					$Campos = $Form->getFormManu();
					$Op = $Form->getOp();
