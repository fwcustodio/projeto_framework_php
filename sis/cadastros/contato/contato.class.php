<?
include_once($_SESSION['DirBase'].'cadastros/contato/contato.sql.php');
...
{	
    ...
    */
    public function cadastrarContato($ObjForm)
    {
        ...
        {
            //Monta Array
            $ArrayDados = array("NomeContato" => $ObjForm->getCampoRetorna("NomeContato".$Chave,true,"Texto"), "ContatoObservacao" => $ObjForm->getCampoRetorna("ContatoObservacao".$Chave,true,"Texto"));
    ...
    */
    ...
    public function removerTipoContato($ContatoCod)
    {
        ...
        $ContatoDadosCod = $Con->execRLinha(parent::getContatoDadosSql($ContatoCod));

        //Dados do Contato
        $Con->executar(parent::removerContatoTipoSql($ContatoDadosCod));
    }
    ...
    public function getContato($ContatoCod, $Id, $ObjForm)
    {
        ...
        if($NCont > 0)
        {
            while ($DadosCont = @mysqli_fetch_assoc($RSCont))
            {
                ...
                $Campos = array("ContatoDadosCod".$Cont, "NomeContato".$Cont, "ContatoObservacao".$Cont);
                ...
                $HtmlContato.= $this->getConteudoContato($CamposCont,$Id,$Cont,$HtmlTipo);
            }
        }
    ...
    public function getConteudoContato($CamposCont, $IdForm, $Cont, $ConteudoTipoContato)
    {
        $Html = '<fieldset id="contato'.$Cont.'" style="width:410px;  border:2px solid #ced8e1; background:#e0e6ed; float:left;"><table width="100%" border="0" cellspacing="0" cellpadding="1"><tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td width="110" align="right" class="textoForm">Contato:</td>
                <td>'.$CamposCont['NomeContato'.$Cont].$CamposCont['ArrayContato'.$Cont].'</td><td><a href="javascript:addTipoContato('.$Cont.',\''.$IdForm.'\')">
                <img src="'.$_SESSION['UrlBase'].'cadastros/contato/figuras/bt_add.gif" border="0" /></a></td></tr></table></td></tr><tr><td>
    ...
    {
        $Html = '<div><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td width="110" align="right" class="textoForm">'.$CamposCont['ContatoCategoriaCod'.$Cont].'</td><td>'.$CamposCont['Contato'.$Cont].'</td>
                 <td><img src="'.$_SESSION['UrlBase'].'cadastros/contato/figuras/bt_remover.gif" border="0" onclick="removeTipoContato(\''.$IdForm.'\',\''.$Cont.'\',this)" style="cursor:pointer" /></td></tr></table></div>';
